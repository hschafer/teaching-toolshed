...
class Canvas:
    def __init__(self, fname, sname='Student', sid='SIS Login ID', dummy_rows=2):
        self.fname = fname
        self.sname = sname
        self.sid = sid
    ...
    def _find_column(self, col, grab_first=False):
        if col in self.canvas:
            ...
            if len(columns) == 1 or (grab_first and len(columns) > 1):
                ...
                raise ValueError(f'canvas_col ({col}) does not uniquely identify one column')
    ...
    def add_grades(self, canvas_col, csv_reader, grab_first=False):
        ...
        self.canvas[canvas_col] = self.canvas.join(csv_reader.scores, how='left')[csv_reader.score_col]

        # Give everyone else a 0
        self.canvas[canvas_col].fillna(0, inplace=True)

    def report_diffs(self):
        df_merged = self.original_canvas.merge(self.canvas, left_on=self.sid, right_on=self.sid, suffixes=('_old', '_new'))

        # Find rows that changed
        diffs = None
        for changed_col in self.changes:
            col_diffs = df_merged[changed_col + '_old'].astype(float) != df_merged[changed_col + '_new'].astype(float)
            if diffs is None:
                diffs = col_diffs
            else:
        ...
        if diffs.sum() == 0:
            ...
            cols = [self.sname + '_old', self.sid] + [col + '_old' for col in self.changes] + [col + '_new' for col in self.changes]
            return df_merged[diffs][cols].copy()

    def export(self, fname=None):
        if fname is None:
            fname = Canvas.export_fname()

        df = self.dummies.append(self.canvas.reset_index(), ignore_index=True, sort=False)
